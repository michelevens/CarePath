...
use App\Models\Lead;
use App\Models\Tour;
use App\Notifications\NewTourRequest;
use App\Notifications\TourRequestConfirmation;
...
class MarketplaceController extends Controller
{
    ...
    public function storeInquiry(Request $request): JsonResponse
    {
        ...
        // Notify the facility's admins + send a confirmation to the
        // inquirer. The whole point of CarePath over the lead-auction
        // competitors is "this gets to a real person at the facility,
        // immediately" — that promise only holds if the notification
        // actually fires.
        $this->notifyTourRequestParties($admission, $facility, 'inquiry');
    ...
    /**
     * Fire the two notifications generated by an inbound tour request:
     *   1. All facility-admin users for this facility (email + in-app)
     *   2. A confirmation email to the inquirer
     *
     * Wrapped in try/catch so a transient SMTP failure or notification
     * row write error can't 500 the public submission — the admission
     * itself was already saved, the worst-case is the manager gets the
     * inquiry via the kanban refresh instead of email.
     */
    private function notifyTourRequestParties(Admission $admission, Facility $facility, string $kind): void
    {
        try {
            // Anyone with role='admin' on this facility's pivot row.
            $admins = $facility->users()->wherePivot('role', 'admin')->get();
            foreach ($admins as $admin) {
                $admin->notify(new NewTourRequest($admission, $facility, $kind));
            }
        } catch (\Throwable $e) {
            \Log::warning('NewTourRequest notify failed', [
                'admission_id' => $admission->id,
                'error' => $e->getMessage(),
            ]);
        }

        if ($admission->inquirer_email) {
            try {
                Notification::route('mail', $admission->inquirer_email)
                    ->notify(new TourRequestConfirmation($admission, $facility, $kind));
            } catch (\Throwable $e) {
                \Log::warning('TourRequestConfirmation notify failed', [
                    'admission_id' => $admission->id,
                    'error' => $e->getMessage(),
                ]);
            }
    ...
    public function bookTour(Request $request): JsonResponse
    {
        ...
        $tour = DB::transaction(function () use ($data, $facility, $startsAt) {
            ...
            return [$tour, $admission];
        });

        // Notify the facility's admins (a tour-booking is a stronger
        // signal than an inquiry — separate kind so the email/in-app
        // copy is appropriate) + confirm to the attendee.
        [$tourCreated, $admissionCreated] = $tour;
        $this->notifyTourRequestParties($admissionCreated, $facility, 'tour_scheduled');
        $tour = $tourCreated;
