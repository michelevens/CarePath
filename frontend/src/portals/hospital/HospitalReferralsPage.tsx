...
export function HospitalReferralsPage() {
  ...
  useEffect(() => {
    let alive = true
    api
      .get<{ data: RefResp }>("/hospital/referrals")
    ...
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Patients you've referred via the widget. Track each through the
          admission funnel.
        </p>
      </div>
          ...
          {STAGE_ORDER.map((stage) => (
            <Card key={stage}>
              <CardContent className="p-3">
                ...
                <div className="mt-1 text-xl font-semibold tabular-nums">{data.by_stage[stage] ?? 0}</div>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center text-sm text-muted-foreground">
            {total === 0
              ? "No referrals yet. Drop the widget code into your discharge-planning system to start sending."
